...
class CalculadoraFilas(tk.Tk):
    ...
    def mostrar_ajuda(self):
        ...
        metricas_container = ttk.Frame(metricas_frame)
        metricas_container.pack(fill="both", expand=True)

        scrollbar = ttk.Scrollbar(metricas_container, orient="vertical")
        scrollbar.pack(side="right", fill="y")

        metricas_text_widget = tk.Text(
            metricas_container,
            wrap=tk.WORD,
            height=6,
            font=("Helvetica", 10),
        ...
        metricas_text_widget.insert(tk.END, metricas_texto)
        metricas_text_widget.config(state="disabled")
        metricas_text_widget.pack(side="left", fill="both", expand=True)

        scrollbar.config(command=metricas_text_widget.yview)
